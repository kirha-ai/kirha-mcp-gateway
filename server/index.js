...
const server = new McpServer({
  name: "kirha-tool-planning-mcp",
  version: "1.0.0",
});

const API_KEY = process.env.API_KEY;
const TOOL_PLANNING_MODE = process.env.TOOL_PLANNING_MODE;
...
const cryptoVerticalSlug = "crypto";
...
function createToolPlanning(query) {
  return fetch(createToolPlanningUrl, {
    ...
    body: JSON.stringify({
      query,
      vertical_id: cryptoVerticalSlug,
    }),
    headers,
  })
    .then((res) => res.json())
...
if (TOOL_PLANNING_MODE === "plan") {
  server.registerTool(
    "create-crypto-tool-planning",
    {
      ...
      `,
      inputSchema: { prompt_query: z.string().describe("") },
    },
  ...
  server.registerTool(
    "execute-crypto-tool-planning",
    {
      ...
      `,
      inputSchema: { plan_id: z.string().describe("") },
    },
